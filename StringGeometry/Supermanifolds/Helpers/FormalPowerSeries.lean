...
/-! ### ODE-based proof infrastructure for det(exp(A)) = exp(tr(A)) -/

section DetExpTraceODE

open Polynomial

/-- Polynomial version of expMatrixNilpotent: entries are polynomials in X. -/
private noncomputable def expPolyMatrix {n : ℕ} (A : Matrix (Fin n) (Fin n) S) (N : ℕ) :
    Matrix (Fin n) (Fin n) (Polynomial S) :=
  Matrix.of (fun i j => ∑ k ∈ Finset.range (N + 1),
    Polynomial.C (algebraMap ℚ S (1 / ↑(k.factorial)) * (A ^ k) i j) * X ^ k)

/-- Constant matrix: lift A to polynomial constants. -/
private noncomputable def constMatrix {n : ℕ} (A : Matrix (Fin n) (Fin n) S) :
    Matrix (Fin n) (Fin n) (Polynomial S) :=
  Matrix.of (fun i j => Polynomial.C (A i j))

/-- Nilpotent exp polynomial: Σ_{k < M} C(c^k/k!) * X^k. -/
private noncomputable def nilExpPoly (c : S) (M : ℕ) : Polynomial S :=
  ∑ k ∈ Finset.range M, Polynomial.C (algebraMap ℚ S (1 / ↑(k.factorial)) * c ^ k) * X ^ k

/-- In a ℚ-algebra, positive natural numbers are units. -/
private lemma natCast_isUnit_of_pos (n : ℕ) (hn : 0 < n) : IsUnit (↑n : S) := by
...
private lemma factorial_cancel_alg' (k : ℕ) :
...
private lemma factorial_cancel_with_pow (k : ℕ) (c : S) :
    algebraMap ℚ S (1 / ↑((k + 1).factorial)) * c ^ (k + 1) * (↑(k + 1) : S) =
    algebraMap ℚ S (1 / ↑(k.factorial)) * c ^ (k + 1) := by
  calc _ = (algebraMap ℚ S (1 / ↑((k + 1).factorial)) * ↑(k + 1)) * c ^ (k + 1) := by ring
    _ = _ := by rw [factorial_cancel_alg' k]

omit [Algebra ℚ S] in
private lemma eval_det_eq' {n : ℕ} (M : Matrix (Fin n) (Fin n) (Polynomial S)) (r : S) :
...
private lemma eval_zero_expPolyMatrix {n : ℕ} (A : Matrix (Fin n) (Fin n) S) (N : ℕ) :
...
private lemma eval_one_expPolyMatrix {n : ℕ} (A : Matrix (Fin n) (Fin n) S) (N : ℕ) :
    (expPolyMatrix A N).map (Polynomial.eval 1) = expMatrixNilpotent A N := by
  ext i j
...
private lemma nilExpPoly_eval_zero (c : S) (M : ℕ) (hM : 0 < M) :
...
private lemma nilExpPoly_eval_one (c : S) (M : ℕ) (hNil : c ^ M = 0) :
...
private lemma deriv_expPolyMatrix_entry {n : ℕ} (A : Matrix (Fin n) (Fin n) S) (N : ℕ)
    (hNil : A ^ N = 0) (i j : Fin n) :
    Polynomial.derivative ((expPolyMatrix A N) i j) =
    ∑ l, Polynomial.C (A i l) * (expPolyMatrix A N) l j := by
  unfold expPolyMatrix; simp only [Matrix.of_apply]
  ...
  conv_rhs =>
    ...
    rw [show (∑ i_1, Polynomial.C (A i i_1 * (algebraMap ℚ S (1 / ↑(k.factorial)) * (A ^ k) i_1 j))) =
      ...
      rw [_root_.pow_succ', mul_apply, Finset.mul_sum]; congr 1; ext l; ring]
  rw [Finset.sum_range_succ]
...
omit [Algebra ℚ S] in
private theorem derivative_det_eq_sum_updateCol {n : ℕ}
...
private theorem sum_det_updateCol_mul_eq_trace_mul_det'
...
private theorem ode_det_expPolyMatrix {n : ℕ} (A : Matrix (Fin n) (Fin n) S) (N : ℕ)
    (hNil : A ^ N = 0) :
  ...
  have hderiv_eq : ∀ j₀ : Fin n,
    ...
    by_cases hj : j' = j₀
    · subst hj; simp only [Matrix.updateCol_self]
      rw [deriv_expPolyMatrix_entry A N hNil i j', hM_def]
      unfold constMatrix; rw [Matrix.mul_apply]; simp [Matrix.of_apply]
...
private lemma poly_ode_zero' {c : S} {P : Polynomial S}
...
private lemma nilExpPoly_ode (c : S) (M : ℕ) (hNil : c ^ M = 0) :
  ...
  | succ N =>
    set T := ∑ k ∈ Finset.range N,
      Polynomial.C (algebraMap ℚ S (1 / ↑(k.factorial)) * c ^ (k + 1)) * X ^ k
    have hL : Polynomial.derivative (nilExpPoly c (N + 1)) = T := by
      ...
      rw [Polynomial.derivative_C_mul_X_pow, show k + 1 - 1 = k from by omega, show
        Polynomial.C (algebraMap ℚ S (1 / ↑((k + 1).factorial)) * c ^ (k + 1) * ↑(k + 1)) =
        Polynomial.C (algebraMap ℚ S (1 / ↑(k.factorial)) * c ^ (k + 1)) from by
    ...
    have hR : Polynomial.C c * nilExpPoly c (N + 1) = T := by
      ...
      have hlast : c * algebraMap ℚ S (1 / ↑(N.factorial)) * c ^ N =
          algebraMap ℚ S (1 / ↑(N.factorial)) * c ^ (N + 1) := by rw [pow_succ]; ring
...
/-- Jacobi's formula: det(exp(A)) = exp(tr(A)) for nilpotent matrices. -/
theorem det_expMatrix_eq_exp_trace {n : ℕ}
  ...
  | succ m =>
    ...
    | succ k =>
      -- ODE-based proof for n ≥ 2
      open Polynomial in
      have hA_nil : IsNilpotent A := ⟨N, hNil⟩
      have htr_nil := Matrix.isNilpotent_trace_of_isNilpotent hA_nil
      obtain ⟨M, htrM⟩ := htr_nil
      set P := (expPolyMatrix A N).det
      set E := nilExpPoly A.trace M
      have hP_ode := ode_det_expPolyMatrix A N hNil
      have hE_ode := nilExpPoly_ode A.trace M htrM
      have hP0 : Polynomial.eval 0 P = 1 := by
        rw [eval_det_eq', eval_zero_expPolyMatrix, Matrix.det_one]
      ...
      calc (expMatrixNilpotent A N).det
          = Polynomial.eval 1 P := by rw [eval_det_eq', eval_one_expPolyMatrix]
        _ = Polynomial.eval 1 E := by rw [hPE]
        _ = IsNilpotent.exp A.trace := nilExpPoly_eval_one A.trace M htrM
